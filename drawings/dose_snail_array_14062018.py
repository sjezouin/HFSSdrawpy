...
if 1==1:
    for ib, valb in enumerate(np.array([5, 5])): # number of snails
        for isp, valsp in enumerate(np.array([6.5, 4, 4.5, 5, 5.5])): # bridge spacing
            pos_y = str(isp*200)+'um'
            pos_x = str(ib*1000)+'um'
            c.key_elt('pads', [pos_x,pos_y], [1,0])
            bridge_spacing = str(valsp)+'um'
            c.pads.draw_dose_test(['100um', '100um'], '500um', '5um', '1.5um', valb, bridge_spacing, '20um', '2um')
...
if 1==0:
    for isp3, valsp3 in enumerate(np.array([3.5, 4, 4.5, 5, 5.5])): # bridge spacing
